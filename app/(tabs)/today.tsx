  ...
  function loadOrders() {
    ...
    fetch(`${process.env.EXPO_PUBLIC_API_URL}/orders/1/today`, {
      ...
      .then((json_data) => {
        if (
          json_data === null ||
          !(typeof json_data == 'object' && 'data' in json_data)
        ) {
              ...
              <View className='w-full p-4 bg-white rounded-lg shadow-md'>
                ...
                <View className='flex flex-row items-center gap-4 mt-4 buttons'>
                  <TouchableOpacity
                    ...
                    className='flex flex-row items-center justify-center w-2/3 p-2 mx-auto bg-green-500 rounded-lg'>
                    <Text className='font-semibold text-white text-md'>
                      {finishQueue.includes(item.id) ? (
