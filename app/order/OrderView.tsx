import { ScrollView, View, Text, I18nManager, Image, TouchableOpacity } from "react-native";
...
export default function OrderView() {
    ...
    return (
        <ScrollView className="w-full h-screen bg-slate-100 p-4 flex flex-col gap-2">
            ...
            <View className="tabs w-full bg-white rounded-md flex flex-row justify-between items-center mt-4 px-2 shadow-md">
                <TouchableOpacity onPress={()=> setTab(0)} className={"flex-1 p-2 " + (tab === 0 ? "border-b-2 border-green-500" : "")} onPress={() => setTab(0)}>
                    <Text className="text-center text-green-500 font-semibold">
                        {I18nManager.isRTL ? "التفاصيل" : "Details"}
                    </Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={()=> setTab(1)} className={"flex-1 p-2 "+ (tab === 1 ? "border-b-2 border-green-500" : "")} onPress={() => setTab(1)}>
